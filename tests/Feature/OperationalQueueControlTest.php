...
class OperationalQueueControlTest extends TestCase
{
    ...
    public function test_operational_screen_centralizes_queue_controls_and_legacy_urls_redirect_to_it(): void
    {
        ...
        $this->get(route('events.operations.waiting.desk', $event))
            ...
            ->assertSee('SKIP')
            ->assertSee('GOTO')
            ->assertSee('LANJUT KE DONOR')
            ->assertSee('Peserta Dilewati')
            ->assertDontSee('Riwayat peserta yang telah menyelesaikan proses.')
            ...
            ->assertSee("canCallNext ? 'Panggil Peserta Berikutnya'", false)
            ->assertSee("skippableTicketFor() ? 'Lewati Peserta Saat Ini'", false)
            ...
            ->assertSee('skippedTickets', false)
            ->assertSee('bg-slate-200 text-slate-500', false)
            ->assertSee('Laki-laki')
            ->assertSee('Perempuan');
